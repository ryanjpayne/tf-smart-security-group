...
module "my-sg2-cidr-source-rule" {
  source = "../../modules/cidr-source-sg-rule"
  sg-id  = module.my-instance-sg2.sg-id

  cidr-source-rules = [
    {index=1, cidr=["0.0.0.0/0"], protocol="tcp", from_port=1, to_port=1, description="1 - Descritpion"},
    {index=2, cidr=["0.0.0.0/0"], protocol="tcp", from_port=1, to_port=1, description="2 - Descritpion"}
  ]
}

# Create an ingress security group rule in which the source is another security group 
module "my-sg2-sg-source-rule" {
  source = "../../modules/sg-source-sg-rule"
  sg-id  = module.my-instance-sg2.sg-id

  sg-source-rules = [
    {index=1, sg=module.my-instance-sg1.sg-id, protocol="tcp", from_port=1, to_port=1, description="1 - Description"}
  ]
}

# Create an ingress security group rule in which the source is the same security group 
module "my-sg2-self-source-rule" {
